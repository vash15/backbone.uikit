...
export default class ListView extends BaseView {
	...
	onRender(rendered) {
		...
		// Container
		let $scrollContainer = this.cache.$scrollContainer = $('<div class="listview-container">').css('display', 'inline-block');
	...
	renderGroups() {
		...
		_.forEach(this.groups, (aGroup) => {
			...
			aListGroupView.$el.appendTo(this.cache.$scrollContainer);
	...
	calculateRowPosition() {
		...
		collection.forEach((aModel, anIndex) => {
			...
			if (aColumnIndex % itemsPerRow === itemsPerRow - 1 || aColumnIndex === this.collection.length - 1) {
				aCumulativePosition += aRowMaxSize;
				if (aModel) {
