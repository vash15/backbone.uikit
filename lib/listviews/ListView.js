...
export default class ListView extends BaseView {
	...
	onRender(rendered) {
		...
		if (this.options.emptyView) {
			this.cache.$emptyText.append(this.options.emptyView);
	...
	getXYFromIndex(index) {
		...
		if (this.options.orientation === ORIENTATION_VERTICAL) {
			xy = {
				...
				y: this.rowPositions[index] || Math.floor(index / this.options.itemsPerRow) * this.options.itemHeight
			};
		}
		else {
			xy = {
				x: this.rowPositions[index] || Math.floor(index / this.options.itemsPerRow) * this.options.itemWidth,
