...
const INITIAL_POSITION_TOP  = -9000;
const INITIAL_POSITION_LEFT = -9000;
...
export default class ListView extends BaseView {
	...
	constructor(options) {
		...
		this.triggerScrollStart = _.bind(this.triggerScrollStart, this);
		this.triggerScrollStart = _.debounce(this.triggerScrollStart, 500, true);
	}
	...
	scrollTop(animated, speed) {
		const newScroll    = 0;
		const newDirection = this.isVertical() ? DOWN_TOP : RIGHT_LEFT;
		this.scrollTo(newScroll, animated, speed, () => {
			this.changeDirection(newDirection);
		});
	}

	scrollBottom(animated, speed) {
		const newScroll    = this.isVertical() ? this.el.scrollHeight : this.el.scrollWidth;
		const newDirection = this.isVertical() ? TOP_DOWN : LEFT_RIGHT;
		this.scrollTo(newScroll, animated, speed, () => {
			this.changeDirection(newDirection);
		});
	}

	scrollTo(newScroll, animated, speed = 200, done = () => {}) {
		const scrollProperty = this.isVertical() ? 'scrollTop': 'scrollLeft';
		...
		this.requestAnimationFrame(() => {
			...
			if (!animated) {
				scroller[scrollProperty] = newScroll;
				// Enable scroll
				this.resume();
				this.requestAnimationFrame(() => {
					this.requestAnimationFrame(() => {
	...
	onClick(e) {
		...
		if ('onSelectItem' in this) {
			...
			// isn't changed
			if (this.options.headerView && !this.headerSize) {
	...
	calculateRowPosition() {
		...
		collection.forEach((aModel, anIndex) => {
			...
			if (this.options.groupBy) {
				if (_.isFunction(this.options.groupBy)) {
					aGroupBy = aModel ? this.options.groupBy(aModel) : null;
				} else {
	...
	getXYFromIndex(index) {
		...
		xy.x += -INITIAL_POSITION_LEFT;
		xy.y += -INITIAL_POSITION_TOP;
	...
	binarySearch(rows, x, y) {
		...
		const headerDeltaX = this.options.orientation === ORIENTATION_HORIZONTAL && this.headerSize ? Math.abs(this.headerSize - this.options.headerSize) : 0;
		const headerDeltaY = this.options.orientation === ORIENTATION_VERTICAL   && this.headerSize ? Math.abs(this.headerSize - this.options.headerSize) : 0;
