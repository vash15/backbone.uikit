...
export default class ListView extends BaseView {
	...
	constructor(options) {
		...
		this.headerSize = this.options.headerSize;
	...
	onClick(e) {
		...
		if ('onSelectItem' in this) {
			...
			// isn't changed
			if (this.options.headerView && !this.options.headerSize) {
	...
	calculateRowPosition() {
		...
		this.containerSize = this.collection.length > this.options.placeholders ? aCumulativePosition + headerSize : aContainterSize + aCumulativeGroupSize; // Math.max(aContainterSize, aCumulativePosition);
	}
