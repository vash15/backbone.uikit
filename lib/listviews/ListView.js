...
export default class ListView extends BaseView {
	...
	calculateRowPosition() {
		...
		this.collection.forEach((aModel, anIndex) => {
			...
			aRow = Math.floor(anIndex / this.options.itemsPerRow);
			...
			if (this.options.orientation === ORIENTATION_VERTICAL) {
				aColumn      = (anIndex % this.options.itemsPerRow);
				aColumnWidth = (this.listWidth / this.options.itemsPerRow);
				...
				aColumn      = (anIndex % this.options.itemsPerRow);
				aColumnHeight = (this.listHeight / this.options.itemsPerRow);
				this.rowItems[aRow].push({
					index: anIndex,
			...
			if (anIndex % this.options.itemsPerRow === this.options.itemsPerRow - 1) {
