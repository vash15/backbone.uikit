...
export default class ScrollBarView extends OsBarView {
	...
	changeStatus(newStatus, options) {
		...
		switch(this.status) {
			case STATUS_SCROLL_DOWN:
				this.$el.addClass('scolled-down');
				this.$el.removeClass('scolled-up');
				if (options.execute)
					this.update(0, true);
				break;
			case STATUS_SCROLL_UP:
				this.$el.removeClass('scolled-down');
				this.$el.addClass('scolled-up');
				if (options.execute)
	...
	update(y, animated = false) {	// If animated is true allow any update
		...
		this.requestAnimationFrame(() => {
			if (animated) {
				// Lock user scroll
				style.overflowScrolling(this.$scroller.get(0), false);
				...
				if ((nameDevice == 'android' && currentDevice.version < 5) || nameDevice == 'windows phone') {
					...
					this.$scroller.animate({
						...
						// Allow user scroll
						style.overflowScrolling(this.$scroller.get(0), true);
					});
				}
			} else {
				// Backround fading
				...
				// this.cache.bottom.style.height = Math.abs(this.options.scrollDownHeight - this.options.scrollUpHeight) + "px";

				// Bottom side scrolling
				style.translate3d(this.cache.bottom, 0, (-this.options.scrollDownHeight * percent), 0);
				...
				style.translate3d(this.cache.bottomInner, 0, (topDistance/2 * percent), 0);		// Position bottomInner centered over the navigation bar
	...
	move(percent, direction, animated) {
		...
		// Override center transition
		this.cache.center.style[ style.getVendorStyle('transition') ] = '';
