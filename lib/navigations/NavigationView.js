...
export default class NavigationView extends BaseView {
	...
	pop(popBarView) {
		...
		let newBarView;
		if (isView(pageView)) {
			if (pageView.getBarView)
				newBarView = pageView.getBarView();
			if (!newBarView && pageView.getNavigationBar)
	...
	onPushedView(view) {
		...
		if (!view.getBarView && !view.getNavigationBar)
			return;

		let barView;
		if (view.getBarView)
			barView = view.getBarView();
		if (!barView && view.getNavigationBar)
			barView = view.getNavigationBar();
