...
search.splitWords = function splitWords(str, removeLastCharacter) {
	...
	for (var i = 0; i < len; i++) {
		...
		if (removeLastCharacter && words[wordIndex].length > 5 && words[wordIndex].length < 24 && (!isCharacter || (i == len - 1) && (quantityCharacters.indexOf(c) > -1 ) )) {
...
search.words = function words(needle, haystack, removeLastCharacter) {
	if ( typeof removeLastCharacter === 'undefined' )
		removeLastCharacter = true;

	if (_searchWordsCache.needle != needle) {
		_searchWordsCache.needle = needle;
		_searchWordsCache.needleWords = search.splitWords(needle, removeLastCharacter);
	}
