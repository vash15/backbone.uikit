...
const ANIMATION_FADE_UP   = 'fade_up';
...
export default class PageView extends BaseView {
	...
	move(percent, direction, animated) {
		...
		switch (this.options.pageAnimation) {
			...
			case ANIMATION_ZOOM_IN:
				...
				break;
			case ANIMATION_FADE_UP:
				this.moveFadeUp(percent, direction, animated);
				break;
		}
		return this;
	}
	...
	/**
	 * Animate the view using ANIMATION_FADE_UP effect.
	 * Never call it directly, always pass through move() method.
	 * @private
	 * @version 2.0.0
	 * @param {integer} percent - Number from 0 to 100 that rapresents the percentage of the animation.
	 ...
	 * @param {boolean} animated - True if the view must move with an animation.
	 */
	moveFadeUp(percent, direction, animated) {
		let self          = this;
		let transform     = '';
		let opacity       = 1;
		let initTransform = '';
		let initOpacity   = 0;
		let timing        = 'cubic-bezier(.17,.53,.36,.86)';

		let easingOut =  BezierEasing(.17,.53,.36,.86);

		switch (direction) {
			case BarView.PUSH:
				initTransform = 'translateY(30%)';
				transform     = 'translateY(0)';
				initOpacity   = 0;
				opacity       = 1;
				break;
			case BarView.DETACH:
				initTransform = 'translateY(0)';
				transform     = 'translateY(-5%)';
				initOpacity   = 1;
				opacity       = 1;
				break;
			case BarView.RESTORE:
				initTransform = 'translateY(-5%)';
				transform     = 'translateY(0)';
				initOpacity   = 1;
				opacity       = 1;
				break;
	...
	onBeforePush() {
		...
		switch (this.options.pageAnimation) {
			...
			case ANIMATION_FADE_UP:
				...
				if (this.options.animated) {
					this.el.style[ getVendorStyle('backfaceVisibility') ]  = 'hidden';
					scaleAndTranslate3d(this.el, '40%', 0, 0, 0, true);
					this.el.style.opacity = 0;
				}
				break;
		}
	...
	onSwipeBackTouchStart(ev) {
		if (!ev || !ev.timeStamp) return; // || !this.isActive
...
PageView.ANIMATION_FADE_UP   = ANIMATION_FADE_UP;
