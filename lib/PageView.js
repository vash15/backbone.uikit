...
var PageView = module.exports = BaseView.extend({
	...
	onSwipeBackTouchEnd: function onSwipeBackTouchEnd(ev){
		...
		function navigationOnSwipeBack(percent){
			...
			if ( ctx && ctx.viewstack )
				ctx.viewstack.onSwipeBack( percent, true );
		};
	},
	...
	onSwipeBackTouchMove: function onSwipeBackTouchMove(ev){
		...
		// Viewstack
		if ( ctx && ctx.viewstack )
			ctx.viewstack.onSwipeBack( percent, false );
	...
	move: function move(percent, direction, animated) {
		...
		var el = this.el;
		el.style[ utilsStyle.transition ]  = '';
		...
		if ( animated ){
			...
			el.style[ utilsStyle.transform ] = transform;
		}
