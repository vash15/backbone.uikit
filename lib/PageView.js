...
function getScreenY(ev){
	var screenY = 0;
	var touch   = getTouch(ev);
	if ( touch && _.isNumber(touch.screenY) )
...
var PageView = module.exports = BaseView.extend({
	...
	onSwipeBackTouchStart: function onSwipeBackTouchStart(ev){
		...
		var screenY = getScreenY( ev );
		if ( isInRect( screenX, screenY, top, left, width, height ) && this.viewport.width > 0 && !this._swipeBackStop ){
	...
	onSwipeBackTouchEnd: function onSwipeBackTouchEnd(ev){
		...
		this.$el.removeClass('swipe-back');
		...
		if ( speed >= this.options.swipeBackVelocityLimit || screenX > (this.viewport.width/2) ){
			pop        = true;
	...
	onSwipeBackTouchMove: function onSwipeBackTouchMove(ev){
		...
		this.el.style[ utilsStyle.transform ]  = 'translate3d(' + screenX + 'px, 0, 0)';
	}
