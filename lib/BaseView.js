...
export default class BaseView extends View {
	 ...
	 * }
	 */
	setDefaultOptions(defaults, moreDefaults) {
	...
	setDefaultsOptions(defaults, moreDefaults) {
		return this.setDefaultOptions(defaults, moreDefaults);
	}
	 ...
	 */
	 addSubView(name, view, state) {
		...
		if (view instanceof BaseView && !view.getState()) {
			if (state) {
				view.setState(state);
			} else if (this.state) {
	...
	changeStatus(newStatus) {
		// console.log('Order bar status:', newStatus);
		if (newStatus === this.status) return;
		const canChangeStatus = this.onExitStatus(this.status, newStatus);
		if (canChangeStatus === false) return;
		const oldStatus = this.status;
		this.status = newStatus;
		this.onEnterStatus(oldStatus, this.status);
	}

	/**
	 * Called when the view enter into a new status.
	 * onExitStatus is called before onEnterStatus.
	...
	/**
	 * Called when the view extis from the current status.
	 * Return false to block the operation.
	 * Override this method.
	 * @version 2.1.5
	 * @param {String} oldStatus The old status
	 * @param {String} newStatus The new status
	 * @return {Boolean} false to block the status change
	 */
