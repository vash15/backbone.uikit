...
export default class TextField extends BaseView {
	...
	className(){ return 'ui-input ui-input-text'; }
	...
	constructor(options) {
		...
		this.options = {
			...
			'formatCharacterCount': options.formatCharacterCount || null
		};
		...
		if ( _.isFunction(this.options.formatCharacterCount) )
			this.formatCharacterCount = this.options.formatCharacterCount;
	...
	formatCharacterCount(fieldLength) {
		if ( _.isNumber(fieldLength) && this.$characterCount.length > 0 && this.textFieldAttributes.maxlength )
