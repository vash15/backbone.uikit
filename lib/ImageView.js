...
export default class ImageView extends BaseView {
	...
	onRender(rendered) {
		...
		if (Math.abs(this.options.orientation) === 90) {
			viewport = {
		...
		if (this.options.orientation !== 0) {
			cssText += '-webkit-transform: rotate(' + this.options.orientation + 'deg);';
			cssText += 'transform: rotate(' + this.options.orientation + 'deg);';
		}
